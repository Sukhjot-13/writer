...
- **Purpose:** Text editing for title/heading/paragraph (auto-grow textarea), Q&A form (renders `QaBlockForm`), separator render, per-block ↑/↓/＋/✕ controls on hover, `/` slash-command popup (FR-2) to convert block type, per-block tags input in the header row (M5, FR-5), and FR-3 keyboard editing: Enter at cursor splits the block, Backspace on an empty block removes it and moves focus up.
...
- **Purpose:** Document cards (title → editor `/?id=`, updated date, block count, tags) + Regenerate (FR-20: POST regenerate route, per-card busy state) + Delete with confirm (DELETE route + `router.refresh()`); client-side sort control (updated/created/title); M5: clickable tag chips row with filter state ("Showing N documents tagged #tag" + clear), and a "⬇ Backup (zip)" button in the tools row that downloads the full-library ZIP from /api/documents/backup (filename parsed from the Content-Disposition header).
