...
- **Functions:**
  ...
  - `generatePDFBuffer(doc, tokens, opts?)` — renders `<PDFDocument>` via `renderToBuffer`; `opts.variant` selects the variant (M6 replaces the old `practice?` flag); non-full variants filter blocks to title + qa. (2026-08-10: vocab grid rows use `t.colors.vocabBg` background + `t.colors.rowBorder` borders — matches the HTML spec.)
...
- **2026-08-10:** chips are deliberately SUBTLE (`border-dashed border-zinc-200 bg-transparent text-[10px] text-zinc-400`, emerald hover) — enrichment is mostly AI-generated, so the add-chips sit quietly until the user wants them.
...
- **2026-08-10:** chip row label renamed to "AI details:" (enrichment is AI-driven; manual entry is the fallback) and styled subtler — same muted chip styling as QaBlockForm.
...
- **2026-08-10:** hover restyled to the emerald-green accent (`hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-600`) — matches the muted chips across the editor.
...
- Remaining: fix the one deferred M5 check (vocab grid row term/def attribution — see suggestions.md), final e2e pass against the running server, then push the unpushed milestones (M5 + UI polish + M6 + bug fixes + design pass) once the user confirms.
