...
- **Types:** `BlockType = "title" | "heading" | "paragraph" | "qa" | "separator"`; `ParagraphContent` (M6: `text`, `format?`, `translation?`, `analysis?`, `vocab?`, `expressions?`); `QaContent` (question, questionTranslation, grammarNote, responseLabel, userAnswer, modelAnswer, answerTranslation, analysis, vocab, expressions, hideTranslation, hideModelAnswer); `setBlockContent`/`replaceBlockType` helpers.
...
- **Exports:** `blockSchema` (discriminated union on `type`; paragraph content += `translation`/`analysis`/`vocab`/`expressions` — M6), `documentSchema`, `saveDocumentPayloadSchema` (`{ doc, html?, instructionsVersion? }` — M6).
...
- **Purpose:** The ONLY PDF engine. Generates A4 PDFs from block data (never from HTML) — no Chrome/Puppeteer anywhere. Styles come from the shared tokens (FR-43): Times-Roman, token colors, print margins (~14mm). `.tsx` because it contains JSX. **M6:** three variants via `PDFVariant` — `"full"` (everything), `"questions"` (shareable practice sheet: title + questions + blank answer areas only), `"my-answers"` (after practice: questions + the user's own answers, no reference answers — send for checking). Paragraph blocks render their translation/analysis/vocab grid in `"full"`. Note: react-pdf v4.6 has no `breakInside: "avoid"` — closest available is `minPresenceAhead` (documented limitation, see suggestions.md).
- **Functions:**
...
- `api/preview/route.ts` — **POST** `{ doc }` → `{ html }` (M6, NEW). Stateless on-demand preview: zod-validates the document, `getTokens()` → `generateTemplateHTML`. The editor posts its CURRENT doc (unsaved edits included) — no save, no artifact written.
...
- **Purpose:** Renders blocks in order with per-block controls and the bottom add-block affordance. Native HTML5 drag-and-drop reordering (M5, FR-3). **M6:** in practice mode the list filters to title + qa blocks only (paragraphs/headings/separators stay in the document but are hidden from practice) and the bottom `AddBlockMenu` is hidden; index/total bounds are computed against the visible list so ↑/↓ work within the practice view.
...
- **M6 done (2026-08-10):** dashboard home, AI-only editable-block conversion, practice master key, on-demand preview + 3-variant PDF, AI enrichment for all text, paste-blocks round-trip. Type-checks clean; build + smoke verification in progress.
...
- Remaining: fix the one deferred M5 check (vocab grid row term/def attribution — see suggestions.md), run the full M2–M5 smoke regression + `unzip -t` on the backup archive, final e2e pass against the running server, then push the two unpushed milestones (M5 + UI polish) with M6 once the user confirms.
