...
2. **Blob caveat — RESOLVED (2026-08-13, round 13):** `BLOB_READ_WRITE_TOKEN` is **no longer needed at all** — the import-html 500 (`Vercel Blob: No blob credentials found`) led to the on-demand rework: no html/pdf/snapshot *file artifacts* exist anymore (html/pdf render on request; the FR-23 snapshot + imported HTML ride on the document as plain fields). `writeFile` survives only for tests/compat. The user can add `MONGODB_URI` with no token and everything works.
