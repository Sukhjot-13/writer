...
Notes: `CHROME_VERSION` and `CACHE_DIR` in `html2pdf/server.js` are hard-coded constants, not env vars. Hard-coded margins (20px in `server.js`) differ from the instructions' 14mm `@page` margin — the writer app should rely on the instructions' print CSS instead (see `suggestions.md`).
